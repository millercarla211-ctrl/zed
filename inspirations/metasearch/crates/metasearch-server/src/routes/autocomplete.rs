...
//!
//! Remote suggestions are disabled by default. When explicitly enabled in
//! config, suggestions are fetched from Google's public Firefox-autocomplete API.
...
async fn autocomplete_handler(
    ...
    let raw_query = params
        ...
        .cloned()
        .unwrap_or_default();
    let query = input::normalize_query(&raw_query, input::MAX_AUTOCOMPLETE_QUERY_CHARS)
        .unwrap_or_default();
    ...
    if !state.settings.search.remote_autocomplete_enabled {
        return (StatusCode::OK, Json(serde_json::json!([query, []]))).into_response();
    }
