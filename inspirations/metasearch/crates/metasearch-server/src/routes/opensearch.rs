...
async fn opensearch_handler(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let base_url = state.settings.server.normalized_base_url();
    let engine_count = state.engine_registry.count();
    let suggestions_url = if state.settings.search.remote_autocomplete_enabled {
        format!(
            r#"
...
<OpenSearchDescription xmlns="http://a9.com/-/spec/opensearch/1.1/"
  ...
  <Description>Privacy-respecting metasearch engine aggregating {engine_count} built-in sources</Description>
