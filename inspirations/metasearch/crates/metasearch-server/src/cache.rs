...
impl SearchCache {
    ...
    /// Build a cache key from the full normalized search query.
    pub fn cache_key_for_query(query: &SearchQuery) -> String {
        let mut categories: Vec<&str> = if query.categories.is_empty() {
            vec!["general"]
        } else {
            query.categories.iter().map(|category| category.as_str()).collect()
        };
        categories.sort_unstable();
        categories.dedup();
        let language = query.language.as_deref().unwrap_or("auto");
        let time_range = query.time_range.as_deref().unwrap_or("any");

        let mut engines = query.engines.clone();
        engines.sort();
