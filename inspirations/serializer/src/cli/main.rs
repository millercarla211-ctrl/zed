...
#[command(
    ...
    about = "Encode JSON to TOON or decode TOON to JSON with a serializer-first CLI",
    long_about = "Serializer CLI - token-efficient structured data conversion for LLM workflows

EXAMPLES:
  serializer --interactive                 # Launch interactive TUI
  serializer -i                            # Short flag for interactive mode

  serializer input.json -o output.toon
  serializer input.toon --json-indent 2
  cat data.json | serializer -e --stats
  serializer input.json --delimiter pipe
  serializer input.toon -d --no-coerce

  serializer input.json --fold-keys       # Collapse {a:{b:1}} to a.b: 1
  serializer input.json --fold-keys --flatten-depth 2
  serializer input.toon --expand-paths    # Expand a.b:1 to {\"a\":{\"b\":1}}

NOTES:
  This CLI focuses on JSON <-> TOON conversion and interactive analysis.
  The dx-serializer syntaxes for compact tool schemas and chat/tool transcripts
  are library-first APIs intended for host and editor integration.",
