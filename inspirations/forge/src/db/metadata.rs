...
impl MetadataDb {
    pub fn create(path: &Path) -> Result<Self> {
        ...
        let write_txn = db.begin_write().context("begin write transaction")?;
        {
            ...
            write_txn.open_table(JOBS_TABLE).context("open JOBS_TABLE")?;
        }
    ...
    pub fn open(path: &Path) -> Result<Self> {
        ...
        // Ensure MIRRORS_TABLE exists for older repos that were init'd before it was added.
        {
            ...
            write_txn.open_table(JOBS_TABLE).context("ensure JOBS_TABLE")?;
    ...
    pub fn replace_mirror_targets(&self, targets: &[(String, Vec<u8>)]) -> Result<()> {
        ...
        write_txn.commit().context("commit replace mirror targets")?;
        Ok(())
    }
    ...
    pub fn store_job(&self, job_id: &str, job_json: &[u8]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            let mut table = write_txn.open_table(JOBS_TABLE).context("open jobs table")?;
