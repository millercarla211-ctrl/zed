...
| Drive/Sources rail | 68/100 | NotebookLM-style source sets and markdown task docs feed agents through rail-visible source sets plus managed Agent attachment receipts, attach-ready counts, source-derived prompt cards, row-level Attach/Review controls, and produced-file proof rows for output existence, receipt, hash, and empty-file warnings | Add row-level receipt drilldowns |
