...
| Forge panel | 40/100 | Code/media snapshots, remotes, sync plans, jobs, restore warnings, approval handoffs, restore approval receipts, restore-target plan receipts, and receipt history are visible through panel-facing contracts; restore preview source rows now expose blocker/risk labels from restore receipts, and read-only Forge history includes restore approval readiness/evidence plus restore-target plan readiness entries | Add governed restore-to-target executor only after mutation approval |
