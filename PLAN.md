...
| `dx_catalog` provider/model archive | 90/100 | `rkyv` + `memmap2` catalog loader/generator powers model picker and routing | Route selected catalog records into real Agent thread model switching and provider execution |
| Universal provider routing | 30/100 | Local, remote, free-tier, premium, and fallback routes work from one catalog | Connect route selections to provider execution with permissioned adapters |
...
Overall implementation status: 54/100.
