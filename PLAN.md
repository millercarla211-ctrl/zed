...
| Check panel | 12/100 | Project score and blockers include a typed read-only score schema for workspace structure, receipt root/file state, attach-ready sources, tool proof receipts, and background-task state | Add validation/visual/deploy proof inputs |
...
| Codex-style rails | 38/100 | Left Sources and right project/task rail are optional, cheap when closed, backed by receipt-producing Agent actions, and now include attachment readiness plus Check score surfaces | Add compact rail controls and restore-warning detail |
| Launch demo package | 50/100 | May 22 sprint demos show speed, local model tools, provider freedom, metasearch, receipt chains, panels, guided proof drafts, source readiness, and Check scoring | Add restore-warning detail, Deploy target registry, and runtime proof |

Overall implementation status: 100/100 for the completed launch-spine set.

Current next 100-point feature set status: 78/100 for DX Native Tool Execution, Restore, Panels, and Launch Demos.
