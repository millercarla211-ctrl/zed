...
| `dx_catalog` provider/model archive | 86/100 | `rkyv` + `memmap2` catalog loader/generator powers model picker and routing | Connect the Agent UI to a generated catalog artifact and render the picker projection |
| Universal provider routing | 24/100 | Local, remote, free-tier, premium, and fallback routes work from one catalog | Connect route selections to real Agent thread model switching and provider execution |
...
Overall implementation status: 50/100.
