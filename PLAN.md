...
| `dx_catalog` provider/model archive | 93/100 | `rkyv` + `memmap2` catalog loader/generator powers model picker and routing | Add permissioned provider execution adapters for catalog-only providers |
| Universal provider routing | 36/100 | Local, remote, free-tier, premium, and fallback routes work from one catalog | Expose catalog-only routes after execution adapters are safe |
...
Overall implementation status: 58/100.
