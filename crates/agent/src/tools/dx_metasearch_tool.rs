...
impl DxMetasearchSourcePackReceiptTarget {
    ...
    fn write_receipt(
        ...
        let receipt = serde_json::json!({
            ...
            "next_action": "Use the latest receipt for Agent source context, then run the future deep-fetch/extract tool for selected source IDs when the short excerpts are not enough."
        });
        ...
        Ok(dx_metasearch_agent_bridge::DxMetasearchSourcePackReceipt {
            ...
            next_action: "Use the latest receipt for Agent source context, then run deep-fetch/extract on selected source IDs when needed.".to_string(),
        })
    }
