...
const DX_RESTORE_APPROVAL_PROMPT: &str = "Prepare a non-mutating DX Forge restore-to-target approval review for this workspace. Use inspect_dx_forge_history and visible restore-preview source rows to summarize the latest safety-policy, backup, backup-manifest, restore-preview, restore-approval, blockers, target path, overwrite risk, rollback evidence, and missing confirmations. If I provide operator approval evidence, use capture_dx_forge_restore_approval to write only a managed approval receipt, then use inspect_dx_forge_history to confirm the restore_approval entry is visible. Do not mutate target paths, overwrite files, delete files, run shell commands, run local servers, or execute restore-to-target actions.";
