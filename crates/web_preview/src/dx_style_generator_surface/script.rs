...
__DX_STYLE_CSS_DECLARATION_DRY_RUN_REVIEW__
    ...
    function reviewPacket(output) {
      ...
      return {
        ...
        context: {
          ...
          css_generator: zedStyleContext?.css_generator || null,
          css_source_edit_safety: zedStyleContext?.css_source_edit_safety || null,
          css_hint_ordinal: zedStyleContext?.css_hint_ordinal || null,
          css_hint_property_pattern: zedStyleContext?.css_hint_property_pattern || null,
          css_hint_property_match: zedStyleContext?.css_hint_property_match || null,
          css_hint_value_contains: Array.isArray(zedStyleContext?.css_hint_value_contains)
            ? zedStyleContext.css_hint_value_contains
            : [],
    ...
    function sourceWriteReadinessPacket(applyGate, output) {
      ...
      if (!bridge.required_source_apply_review_receipt_fields.includes("css_declaration_hint")) {
        missingRequirements.push("write_bridge_missing_css_declaration_hint_receipt_field");
      }
      ...
      if (!bridge.required_source_apply_review_receipt_fields.includes("post_write_digest_verification_plan")) {
        missingRequirements.push("write_bridge_missing_post_write_digest_verification_plan_receipt_field");
      }
      ...
      if (!bridge.required_source_apply_review_receipt_fields.includes("native_mutation_writer_preflight")) {
        missingRequirements.push("write_bridge_missing_native_mutation_writer_preflight_field");
      }
      ...
      if (bridge.runtime_validation_required === true && !bridge.required_runtime_proofs.includes("successful native writer dry-run replay")) {
        missingRequirements.push("write_bridge_missing_native_writer_replay_runtime_proof");
      }
      if (bridge.runtime_validation_required === true && !bridge.required_runtime_proofs.includes("post-write source digest verification")) {
        missingRequirements.push("write_bridge_missing_post_write_digest_runtime_proof");
      }
    ...
    function nativeMutationWriterPreflightPacket(applyGate) {
      const bridge = editorWriteBridgeReviewPacket(applyGate);
      return {
        schema: "zed.web_preview.dx_style.native_mutation_writer_preflight.v1",
        status: "not_performed_in_web_preview",
        reason: "Native mutation writer preflight requires the trusted Zed-side source writer and authorized runtime validation.",
        source_mutation_enabled: sourceApplyMutationEnabled,
        editor_write_bridge_can_apply: bridge.can_apply,
        editor_write_bridge_can_mutate_source: bridge.can_mutate_source,
        mutation_write_receipt_schema: bridge.mutation_write_receipt_schema,
    ...
    function runtimeValidationReceiptTemplatePacket(applyGate) {
      const bridge = editorWriteBridgeReviewPacket(applyGate);
      return {
        schema: "zed.web_preview.dx_style.runtime_validation_receipt_template.v1",
        status: "not_performed_in_web_preview",
        runtime_validation_receipt_schema: bridge.runtime_validation_receipt_schema,
        required_runtime_validation_receipt_fields:
          bridge.required_runtime_validation_receipt_fields,
        source_apply_receipt_schema: sourceApplyReceiptSchema,
        required_runtime_proofs: bridge.required_runtime_proofs,
    ...
    function updatePreview() {
      ...
      const contextLines = zedStyleContext ? [
        ...
        Number.isInteger(zedStyleContext.css_hint_ordinal) ? `css_hint_ordinal: ${zedStyleContext.css_hint_ordinal}` : null,
        zedStyleContext.css_hint_property_pattern ? `css_hint_property_pattern: ${zedStyleContext.css_hint_property_pattern}` : null,
        zedStyleContext.css_hint_property_match ? `css_hint_property_match: ${zedStyleContext.css_hint_property_match}` : null,
        Array.isArray(zedStyleContext.css_hint_value_contains) ? `css_hint_value_contains: ${zedStyleContext.css_hint_value_contains.length}` : null,
