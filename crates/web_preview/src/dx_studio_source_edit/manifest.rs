...
enum ManifestSurfaceSelection {
    Unique(Value),
    Ambiguous(Vec<Value>),
}

fn select_manifest_surface(
    surfaces: Vec<Value>,
    selection: &Value,
...
fn matching_surfaces(manifest: &Value, contract: &Value, selection: &Value) -> Vec<Value> {
    [
...
fn operation_contracts_for_surface(
    contract: &Value,
    surface: &Value,
    selection: &Value,
) -> Vec<Value> {
...
fn operation_ids_for_surface(surface: &Value, operation_contracts: &[Value]) -> Vec<String> {
    unique_strings(
...
fn surface_summary(surface: &Value) -> Value {
...
fn operation_summary(operation: &Value) -> Value {
    let mut summary = Map::new();
    copy_string(
        operation,
        &mut summary,
...
fn edit_contract_summary(contract: &Value, manifest_path: &str) -> Value {
    let mut summary = Map::new();
    summary.insert(
        "manifest".to_string(),
