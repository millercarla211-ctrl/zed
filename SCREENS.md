...
Awesome, you did really well, but please fix these problems first:
- When I am creating multiple terminals, it's showing multiple terminal instances on our screen dock, but we don't need to do that, as the terminal item is the one we need. We don't care about how many terminals are in that terminal screen.
- When we are on our browser screen and clicking on the new item, it's not creating a new browser instance tab. Please make sure it is a new browser instance tab.
- When we are on the code input place code screen, please make sure that when we click on the ad icon, it creates a new untitled file.
- After doing all of this, currently the screen dock is small and the items are small, but maybe there's a separator. The separator's height is much longer, and that's why the screen dock is still consuming almost all of the height. Please make sure to make all the icons container and the separator be on a small height so that the screen dock has a proper, less than the full height, so it will look better.

Yes yes, you are right. Currently only the left and right border is visible but I want to render the full border correctly and the border radius will be as low as possible.
For the second question, the answer is that you are right about that and it will create an actual browser and terminal. Here is the thing: the editor can already render browser and terminal screens so you just have to make sure that on the different, separated screen they can only render their own category stuff. Like previously from one screen we can create tabs for browser, terminal, and code editor at the same time but from now on I want to give every screen as a category and every category will only be able to add tabs on their own.
No other things. By default there is a code screen. In the code screen we should only be able to create code editor related screens but previously we were able to create browser and terminal screens. Currently users have to show the code editor screen and make sure that in that screen it shows browser tabs. In the browser screen you have to just spam the code editor screen and in that screen users have to make sure that browser tabs are always created. For the terminal screen it is the same. You just have to spam the code screen and in that code screen users have to make sure that only terminal tabs should be creatable.

Even you are saying that our screens are showing correctly but they are not. When we are on the code screen it's showing correctly but when we are clicking on the terminal, a screen on the left side is created that is completely blank. It should be a full-width screen by default, just the code screen with only terminal tabs. For the browser tab too it should just be the code strings browser tab but in there only the browser tab should be creatable. It should look like this but currently it's just blank with a small width instead of the default full width. 

In our screen drop we have a border on the screen drop. If I increase the height of the div, it's not showing the full border; it's like clipping it. Maybe the parent toolbar has an item center with a fixed height so that's why it's clipping all the items that go beyond that height. Please remove that fixed height so that I can increase the height of our screen drop. Please look.

In our title bar we were positioning the screen dock by the whole screen center, which is right and all. Now please make it in the center around the title bar, like it respects the position of the left side and the right side of the title bar. Previously we are not doing that so please make sure to respect both sides and put them in the center, not specifically in the center of the whole code editor. 
