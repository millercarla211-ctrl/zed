...
test("Zed Style rail keeps GPUI as the shell and Web Preview as the generator host", () => {
  ...
  assert.match(snapshot, /dx_style_generator_url_with_context/);
...
test("Web Preview owns the DX Style generator surface action", () => {
  ...
  assert.match(webPreviewView, /dx_style_generator_url\(\)/);
  assert.match(webPreviewView, /dx_style_generator_url_with_context/);
  ...
  assert.match(sourceApply, /context source_digest does not use fnv1a64/);
