...
- Added gated provider/model/catalog refresh actions on the managed provider catalog row so the Zed surface exposes the current `dx-agents providers list --json`, `dx-agents models list --json`, and `dx-agents providers catalog regenerate --json` bridge commands without credential import.
